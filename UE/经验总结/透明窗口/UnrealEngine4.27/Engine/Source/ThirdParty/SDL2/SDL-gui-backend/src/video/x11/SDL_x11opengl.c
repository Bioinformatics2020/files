...
XVisualInfo * selectVisualInfoWithAlpha(_THIS,GLXFBConfig* fbConfigs, int fbconfigCount, Display* display, GLXFBConfig* outFBConfig);
...
X11_GL_GetVisual(_THIS, Display * display, int screen)
{
	...
	_this->gl_data->glXGetVisualFromFBConfig) {
		...
		//        GLXFBConfig fbconfig = 0, *lfbConfigs = _this->gl_data->glXChooseFBConfig(display, DefaultScreen(display), visualData, &numfbconfigs);


		vinfo = selectVisualInfoWithAlpha(_this, framebuffer_config, fbcount, display, NULL);
        ...
        if (_this->gl_config.major_version < 3 &&
            ...
            if (!_this->gl_data->glXCreateContextAttribsARB) {
                ...
                if (_this->gl_data->glXChooseFBConfig) {
                	...
                	selectVisualInfoWithAlpha(_this, framebuffer_config, fbcount, display, &finalFBConfig);
            
                    if (framebuffer_config) {
                        context = _this->gl_data->glXCreateContextAttribsARB(display,
                                                        finalFBConfig,//framebuffer_config[0],
...
//    GLXFBConfig fbconfig
    ...
    {
        vinfo = (XVisualInfo*) _this->gl_data->glXGetVisualFromFBConfig(display, fbConfigs[i]);
        if (!vinfo)
            continue;
